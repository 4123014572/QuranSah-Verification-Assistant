<?php
    ...
    for($page=1; $page<605; $page++) { //max 605
        ...
        while($rsNextAya = mysql_fetch_assoc($rs)) {
            $content .= $rsNextAya['text'] . arabicNumber($rsNextAya['aya']);
        }
        
        $myfile = fopen('../sura_plain/Sura '.$rsNextPage['sura_name'].', Aya '.$rsNextPage['minaya'].'-'.$rsNextPage['maxaaya'].'.html', 'w') or die("Unable to open file!");
